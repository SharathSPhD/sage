...
## F-0008 — CAISO LMP series read as time-REVERSIBLE at first instrument contact (certified null)

- Date: 2026-08-11
- Instrument / experiment: `kld_epr` on discretized price series via `experiments/electricity_reading.py` (unit domains.electricity, loader slice)
- Config: `config/experiments/electricity_reading.yaml` (seed 20260811; SP15 hub; DAM hourly 35d n=840; RTM 5-min 14d n=4032; 6 quantile bins; 200 FT surrogates)
- Expected / observed: expected the diurnal ramp to register as measurable irreversibility, at least in RTM. Observed: **no detection in either market** — value-space KLD sits exactly on its phase-randomised null (DAM 0.0121 vs null median 0.0121 — the predicted structural blindness: a periodic drive retraces its 1-D price path), and the (price, Δ-sign) phase embedding reads AT/BELOW its null in both markets (DAM 0.045 vs median 0.050; RTM 0.047 vs median 0.285). The series are indistinguishable from linear (time-reversible) processes with the same spectra at this n and binning.
- Methods lesson (the real content): two wrong nulls were caught en route — a plain shuffle null fails twice (destroys persistence; the Δ-sign embedding of an iid series is structurally asymmetric, null median 0.15), and value-space discretization is *provably* blind to loop irreversibility (unit test: noisy sine reads 6e-5 in value space, 0.032 embedded). Phase-randomised (FT) surrogates are the canonical null and are now the standard in the experiment.
- Chase status: chasing. (1) finer binning + longer RTM windows (power); (2) ramp-asymmetry-sensitive statistics (third-order, visibility-graph style) as alternative meters; (3) spike-conditioned windows (scarcity hours) where asymmetry should concentrate; (4) other nodes/hubs.
- Resolution / follow-up: recorded as the programme's first certified real-data null — the instruments do not hallucinate irreversibility on market data, and detection claims elsewhere inherit that credibility. p3_noneq data section when the chase resolves.
