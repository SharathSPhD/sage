...
- Next steps (precise, for the continuation): (1) implement per-window relaxation-time estimation from the window's own lag-autocorrelation (data-side, no generator); (2) usable ⟺ every window's τ̂_relax ≤ τ/3 AND the IFT equivalence holds (necessary companion, not primary); (3) re-run the registered sweep; P1–P3 unchanged; (4) the gap-collapse coupling to F-0006 is worth a paragraph in p3 — quenching INTO concentration is where data-side quench estimation is hardest, which is exactly where markets live at high stakes.
