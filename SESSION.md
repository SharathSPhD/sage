...
## 2026-08-12 (cont.) — release-pipeline hardening + a real protocol gap CI found

- pypi-release made idempotent and proven GREEN by dispatch: the tag's first run failed with "400 File already exists" (0.1.0 was published by hand when the token arrived), and uv's --check-url semantics differ by version (0.11 tolerates, 0.12.3 errors), so the workflow now matches that ONE benign error explicitly and fails on anything else.
- CI strict-typing surfaced a genuine defect nothing else caught: `BiddingOracle` never implemented `response_matrix`, so the electricity plugin did NOT satisfy the PayoffOracle protocol it advertises. Implemented (central differences in offer-grid index space, edge-clamped), `ENGINE` given its Literal type, and three runtime conformance tests added (isinstance against the protocol, shape/finiteness, edge clamping) — same blind-spot lesson as F-0018: the gates checked what they were pointed at.
- Full strict mypy clean across 65 source files; ruff clean; 17 electricity tests green.
- The typed `ENGINE` then exposed a second latent defect: the plugin-boundary hook's regex only matched the BARE declaration, so annotating it silently voided the plugin's ADR-0008 allowance and blocked a legitimate import. Hook regex widened to accept both forms (the rule itself unchanged — a wrong engine is still blocked), with four regression tests; one of them found that pricing/security/sports declare no ENGINE at all, so the invariant is now scoped precisely to domains that construct a DomainPlugin (loader-only modules exempt by construction, and the test documents which is which).
- Status at handoff: pypi-release GREEN, API image GREEN, Docs GREEN; CI + Gates still running on 98dafa3 (long suites — full strict typecheck across 3 Pythons and the whole gate battery). All three defect classes were verified locally first (ruff clean, strict mypy clean on 65 files, electricity 17/17, hook tests 24/24).

## 2026-08-12 (cont.) — CI Gates had NEVER passed: the aggregate check found what per-unit runs couldn't

- The Gates workflow shows 16 failures / 0 successes: `run_gates.py --check` runs ALL units at once, and my sessions only ever ran single units. It flagged a genuine api.core regression (mypy: `load_sioux_falls` not explicitly re-exported by domains.congestion, plus a `sorted(key=dict.get)` overload) — fixed; **api.core now GREEN**. Same blind-spot family as F-0018: the checks only cover what they're pointed at.
- R7 re-review returned GRANTED-CONDITIONAL demanding a shuffle control. Implemented as a seeded permanent artifact metric — and it overturned BOTH readings: 0/7 months are drift-attributable (5 anomalies survive the shuffle; May's flag FLIPS clean→anomalous under a physically-null permutation, proving monthly reads at n≈30 are sampling noise). F-0017 rewritten a THIRD time to claim only (a) the in-scope closed-loop affinity 7.0 nats/day [6.22, 8.50], descriptive, and (b) that the monthly scan answers nothing. The reviewer's own Jan–Mar/Apr–Jun split did not reproduce; both readings are on the record and the verdict is back with them.
- R7 GRANTED on round 3 → **domains.electricity.quench is the 24th green gate**. The reviewer verified the closure alignment (mean_y 7.000012), the block-bootstrap width ratio (1.57× for ρ≈0.6), and traced their own conflicting one-off shuffle to RNG-state evolution (their draws followed the 200-iteration bootstrap) — confirming the seeded artifact control as authoritative. Three headline revisions, all on record; the surviving claim is deliberately narrow.
- **The aggregate gate suite is GREEN for the first time**: local `run_gates.py --check` → 24/24 units GREEN, 0 RED, 0 regressions (25 gate files = 24 units + the schema). Coverage reconciled: finite.core, the Hodge decomposition and apps/web are checked under `finite.response.reciprocity`'s paths, so nothing lost coverage when the early units were folded into milestone gates. CI's Gates job runs this same command, so its 16-failure streak should end on this commit.

## 2026-08-12 (cont.) — R8: small-n REFUSED, and the mechanism is the finding (F-0019)

- Design red-teamed BEFORE results existed; its 2 blocking + 3 must-address objections were applied while the first run was STOPPED mid-flight (verdict never produced or read). Amendments: S1b width guard (coverage alone is gameable — `boot_se` now exposed), held-out confirmation on a disjoint seed stream (the smallest-of-15 rule passes by chance otherwise), transfer validation on a second GAME (with a recorded deviation: the reviewer's τ=16 is gate-refused, so coverage there would be vacuous), S3 documented as a joint physics+SE test, t_widened documented and warned as a heuristic comparator.
- **Verdict REFUSED at every tested n** — the n ≥ 200 floor stands. The mechanism is the deliverable: interval COVERAGE holds at every n down to 20 (19–20/20, width ~1.9× SE, all three constructions), so the estimator's NUMBER is fine on a month of data; the DECISION machinery is what fails (gate agreement 10/20 at n=20–30 → 17/20 at n=200; permutation flag-flips 4–6 → 1). The estimate converges ~10× faster than the verdict certifying it — mechanistically explaining F-0017's flicker.
- Stated against my own criteria: n=200 also misses S2 (17 vs 18) — either the bar is slightly strict or the existing floor is marginal on decision stability; left unresolved for the reviewer. Chase item with a sharp shape: monthly market numbers may be quotable if the settling verdict is imported from the in-scope full-window read — a separate registration, not a conclusion.
- Results red-team returned GRANTED-CONDITIONAL with three conditions, all now met and each changing the finding: **C-1** — against the generator's EXACT settling status (no sampling noise) n=200 scores 19/20 and PASSES, so the earlier 17/20 was the finite-sample reference's own noise and the existing floor is SOLID (small-n still fails at 10/20, so the refusal is reference-independent); **C-2** — a within-group permutation preserving the SE split collapses flag flips 6→0 (n=30), 5→1 (n=50), 3→0 (n=100), proving the small-n instability is substantially an ARTIFACT of the `i::4` split, so the refusal is implementation-bound and the gate can be fixed rather than the floor raised (→ R9); **C-3** — the import-the-full-window-verdict shortcut is rejected in writing with its reason (a slow subset hides inside a passing aggregate).
- R8 GRANTED → **thermo.hs_estimator.smalln is the 25th green gate**. Two independent reviews (design pre-results, then results), seven objections, all dispositions recorded. The unit's deliverable is a REFUSAL whose value exceeded the extension it was chasing: the n≥200 floor is now better-supported than before R8 ran (exact-reference 19/20), and the reason monthly windows fail is localised to a fixable implementation detail rather than the physics.

### R9 — thermo.hs_estimator.gate_se: the fix works, the floor doesn't move (2026-08-12)

- Registered G1–G5 BEFORE any code ran (`60619d8`), then pinned G5's window aggregation and oracle seed stream while still blind (`d21059a`) — both verified landed via `git log`. G5 was added at registration specifically to pre-empt the objection I expected ("is the new SE more accurate, or merely narrower?") rather than leave it to the review; G4 was registered as ABSOLUTE because G2 is one-sided (the primary truth is "settled", so shrinking the SE to zero would score 20/20).
- Implementation: `relaxation_gate` extracted as a PUBLIC standalone instrument; τ̂ reduced to sufficient statistics so leave-one-out becomes subtraction — an O(nT) jackknife instead of O(n²T), cheaper than the split it replaces, and exact order-invariance becomes a theorem rather than a hope. Refactored `split` verified **bit-identical** to the original (max |Δτ̂| = max |ΔSE| = 0.0 across n ∈ {9…200}, settled and unsettled), so no recorded R7/R8 verdict could move silently. Thermo suite 56/56.
- **Verdict REFUSED — no candidate passes all five, and the incumbent fails three.** G1 SOLVED outright (jackknife/delta/bootstrap all 0/20 flips vs split's 7/6/0 — R8's C-2 prediction confirmed). G5 discriminated as designed and `bootstrap` won it (0.18–0.29 from an independently measured oracle vs split 0.44–0.49, jackknife 0.32–0.52). G4 clean for all (60/60). **G2 fails at n=30 for everyone**; `bootstrap` misses that ONE cell (12/20 vs 18) with all four other criteria passed.
- The mechanism is the finding: τ̂'s own across-seed SD is 0.967 at n=30 (~35–40% of τ̂), so an SE that is *accurate* must report that variance and must therefore refuse genuinely settled holds. **This partly RETRACTS my own extrapolation in F-0019**: R8 had fused two separable failure modes — flag *stability* (implementation-bound, now fixed completely) and gate *accuracy* at small n (a real property of τ̂). Monthly quotability needs a lower-variance relaxation-time estimator, not a better error bar.
- Real-data obligation (R7's CAISO panel, flip rate over 20 null day-order shuffles/month): 0.214 → 0.050, month 06 collapsing 17/20 → 0/20, and admission MOVES (month 07 refused under split, admitted under all alternatives) with the estimate provably identical.
- **Two corrections to already-granted work, recorded rather than edited away.** F-0017's "1 of 7 months UNSTABLE" came from a single permutation per month and is not reproducible across draws (my own v1 of this diagnostic inherited the same flaw and measured 0 under every method, including the incumbent — a vacuous check I had to rebuild as a rate); and its per-month refusals were not SE-method-invariant.
- **A SECOND order-dependence found outside R9's scope**: the CI/IFT bootstrap draws resample indices from `default_rng(0)`, so permuting trajectories re-rolls the interval while `mean_y` stays exact to six decimals (verified on month 05: IFT upper bound swings 0.970–1.246, flag flips). Not fixable by reseeding — the flag is a hard boolean thresholded on a Monte-Carlo interval sitting at its threshold. Captured as a characterisation test so the eventual fix cannot land silently. That is R10.
- Side hardening, found by R9's own scripts failing: `strataq_bench` now forbids extra fields (pydantic silently DROPPED three misspelled kwargs — an artifact could land quietly impoverished, the F-0018 pattern a third time) and serialises NaN as `NaN` rather than `null` (two committed artifacts still carry unreadable nulls). Verified all 39 BenchmarkResult artifacts still validate.
- **Registered-rule gap flagged, not resolved unilaterally**: the rule says the default stays `split` if nothing passes, but did not anticipate the incumbent failing three criteria while `bootstrap` fails one. Default left at `split` per the letter; the recommendation to adopt `bootstrap` goes to the red-team with the evidence.

## 2026-08-13 — R11 + product surfaces merged to main (operator-directed)
- **Merged un-gated.** See ADR-0014. R11 verdict is INDETERMINATE, its red-team verdict was WITHHELD-then-corrected-but-not-re-reviewed, and no gate file exists. A deliberate, recorded exception, not an oversight.
- **Library**: `strataq.diagnose()` / `Diagnosis`, `strataq.viz`, `strataq.games`; `GameThermoRead` gained a warnings field. 50 new tests. Six API mismatches fixed while wiring, one substantive: the EPR side-test used value > 0, so an exact potential game returning 2.16e-31 classified as a driven landscape.
- **Criticality defect, found only by running on the Spark**: `coordination(2,3,bonus=2.0)` at lambda=1.5 sits on the pitchfork, rho(SB)=1+4e-16. R read through a singular resolvent gave 0.0659 on aarch64/jax 0.11 and 0.0 on x86-64/jax 0.10 — a POTENTIAL game diagnosed as a whirlpool. `diagnose()` now consults `strategic_spectrum` and refuses R at criticality. PROGRAMME v3 section 8.5 asked for exactly this; the instruments had the guard, the facade did not consult it.
- **App**: `/diagnose` route. Its column heuristic originally picked the highest-variance numeric column — always the row-index counter, a perfect monotone ramp — so every example would have read as detected regardless of the data. Fixed to exclude constant-step columns and rank by coefficient of variation.
- **Docs**: `research/DIRECTION_v4.md` (names the claim the programme had mislabelled as a refuted repair hypothesis), `docs/product/PRODUCT_v1.md`, `docs/product/COMPETITIVE_POSITION.md`, `papers/PAPER_PROGRAMME_v2.md`, `papers/p2_plane/` (18pp, six figures).
- **Local CI on the Spark before merge**: ruff clean, ruff format clean, mypy strict clean over 70 files, full pytest suite passed.
- **Next action**: re-review R11, write its gate, register the paired-controls unit (O-3/O-6), run K4, test N>2.
