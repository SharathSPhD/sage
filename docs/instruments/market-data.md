...
## The trading day as a repeated quench — refuted, informatively (unit domains.electricity.quench, F-0017)

The certified quench meter, pointed at 211 CAISO DAM day-pairs, delivered its verdict month by month: **five of seven months individually fire the anomaly detector** (IFT as low as 0.002) — the trading day is *not* one repeated stationary protocol; seasonal drift breaks the premise, and the instrument refused it rather than quoting through it. What survives as a descriptive number is the **closed daily-loop affinity ≈ 7.0 nats/day** (day-block bootstrap [6.22, 8.50]; the first framing half-counted the circular protocol — the red-team's wrap-around catch — and treated serially-correlated days as independent; both corrected). Quoted descriptive, not certified: 6 states sits outside the estimator's validated scope, and the IFT companion's open-chain identity does not constrain closed cycles. Against F-0009's ~1.1 nats/day stationary EPR on the same market: different observables, sensible ordering. The certified-scope market read this framing wanted requires a small-n extension (monthly windows) or a drift-corrected protocol model — both on the chase list. Artifact: `day_quench_read.json`.
