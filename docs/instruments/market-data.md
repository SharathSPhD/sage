...
## The trading day as a repeated quench — what a real market read actually supports (unit domains.electricity.quench, F-0017)

Two adversarial rounds turned this unit's headline over twice, and the third version is the narrow one. What survives: the **closed daily-loop affinity of 211 CAISO DAM day-pairs, ≈ 7.0 nats/day** (day-block bootstrap [6.22, 8.50] — the first framing half-counted the circular protocol and treated serially-correlated days as independent, both corrected), quoted as *descriptive* because 6 states extends rather than inherits the estimator's 4-state certification and because the fluctuation-theorem companion cannot constrain a closed cycle.

What does **not** survive: the seasonal-drift story. A seeded shuffle control — re-read each month with day order randomised, destroying temporal drift while preserving thinness and price distribution — attributes **0 of 7 months** to drift, and one month's anomaly flag *flips* under a permutation that changes nothing physical. At n ≈ 30 days a monthly read is sampling noise, so it supports no causal statement in either direction. Answering "is the trading day one repeated quench?" needs a small-n extension of the certification or coarser windows over far more days; the honest present answer is that this design cannot tell. Artifact: `day_quench_read.json` (the shuffle control is a permanent metric, reusable by any future repeated-protocol claim on real data).
